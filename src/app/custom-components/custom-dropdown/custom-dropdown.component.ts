...
export class CustomDropdownComponent {
  @ViewChild('selectInput') public selectInputElement: ElementRef<HTMLInputElement>;
  ...
  public onMouseMove(event: MouseEvent) {
    ...
    const index = listItemElements.findIndex((listItem: ElementRef) => listItem.nativeElement === (event.target as HTMLElement).parentElement);
  ...
  @HostListener('keydown', ['$event']) public onKeyDown(event: KeyboardEvent) {
    if (this.isDropdownOptionsShown()) {
      ...
      } else if (event.key === 'ArrowDown' && this.hoverIndex + 1 <= this.options.length - 1) {
        this.hoverIndex += 1;
        this.scrollOptionIntoView(1);
  ...
  public toggleDropdown() {
    if (!this.disabled) {
      this.isDropdownOptionsShown() ? this.closeDropdown() : this.openDropdown();
    }
  }
  ...
  public openDropdown() {
    ...
    this.hoverIndex = this.value ? this.options.findIndex((option: string) => option === this.value) : 0;
  ...
  private scrollOptionIntoView(direction: number) {
    ...
    const visibleElements = this.getVisibleElements((targetElement as HTMLElement).parentElement).filter((element: HTMLElement) => element.tagName === 'LI');
    ...
    if (!visibleElements.includes(targetElement)) {
      if (direction === -1) {
        ...
        const elementThatShouldBeVisible = itemElements[this.hoverIndex - 5]?.nativeElement;
  ...
  private getVisibleElements(scrollableList: HTMLElement): HTMLElement[] {
    ...
    const elements = Array.from(scrollableList.querySelectorAll<HTMLElement>('*'));
  ...
  private getDropdownOptionsElement(): HTMLElement {
    return (this.elementRef.nativeElement as HTMLElement).getElementsByTagName('ul')[0];
  }
}
